...
#' @param u Matrix or Data-Frame of numerical values containing the coordinates
...
#'
#' @seealso [clustBRIC()], [filter_outliers()], [median_rec()], [median_mv()]
#'
#' @examples
#'
#' # Illustrative data
...
depth_values <- function(data, u = NULL, method = "Projection", warnings = FALSE) {
  ...
  if (is.null(u)) {
    ...
    if (!is.numeric(u)) {
      ...
      if (ncol(u) != ncol(data)) {
        stop("`u`, when provided, must contain the same number of columns as `data`")
      }
    }
  }
