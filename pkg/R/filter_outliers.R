...
#'   exceeding the threshold, for plotting purpose (see [plot.BRIC.Filtering()])
...
#'   and `distType`)} \item{`distances`}{Distances of each sample from `data` to
#'   the `center` provided} \item{`p.values`}{P.Values of the test at each
#'   iteration} \item{`index.p.values`}{Subset size corresponding to each
#'   P.Value, for plotting purpose} \item{`selected`}{Indices of the samples
...
#'
#' @seealso [plot.BRIC.Filtering()], [print.BRIC.Filtering()], [clustBRIC()],
...
filter_outliers <- function(data, center, test = "Mardia", threshold = 0.05, distType, trimmedPerIteration = 1, debug = FALSE, warnings = FALSE) {
  ...
  if (!is.numeric(center) || length(center) != ncol(data)) {
    stop("Parameter `center` must be a numerical array of same dimension as `data` rows")
  }
  ...
  if (is.character(test) && (tolower(test) %in% tolower(opts_test))) {
    ...
    stop("Parameter `test` should be a user-defined function or one of \"", paste(opts_test, collapse = "\", \""), "\"")
  }

  if (is.function(test)) {
    testType <- "Custom"
  } else if (test %in% c("DIP")) {
  ...
  if (distType == "Euclidean" || is.null(distances)) {
    ...
    distances <- integer(nrow(data))
    for (i in 1:nrow(data)){
      distances[i] <- sqrt(sum((data[i,] - center) ^ 2))
    }
  }
  ...
  if (testType == "Unimodality") {
    ...
    output$p.values <- test_multinormality(distances, test = test, data = data, center = center, threshold = threshold, warnings = warnings)
  } else {
    output$p.values <- test_custom(data, test, center = center, distances = distances, warnings = warnings)
  }
  output$index.p.values <- length(selected)

  # Remove the furthest sample(s) until reaching multinormality/unimodality
  while ((length(selected) - trimmedPerIteration > ncol(data)) && (is.na(utils::tail(output$p.values, 1)) || utils::tail(output$p.values, 1) < threshold)) {
    ...
    if (testType == "Unimodality") {
      ...
      output$p.values <- c(output$p.values, test_multinormality(distances[selected],test = test, data = data[selected, ],
                                                                center = center, threshold = threshold, warnings = warnings)
      )
    } else {
      output$p.values <- c(output$p.values, test_custom(data[selected,], test, center = center,
                                                        distances = distances[selected], warnings = warnings)
      )
    }
  ...
  while (debug && ((length(selected) - trimmedPerIteration)) > ncol(data)) {
    selected <- utils::head(selected, - trimmedPerIteration)
    if (testType == "Unimodality") {
      output$p.values <- c(output$p.values, test_unimodality(distances[selected], test = test, warnings = warnings))
    } else if (testType == "Normality") {
...
test_unimodality <- function(values, test = "DIP", warnings = FALSE) {
  ...
  p.value <- withCallingHandlers(
    ...
    error = function(e) {
      if (warnings) {
        if (warnings) {
          warning("Error caught in test_unimodality() with test=\"", test, "\" and ",
                  length(values), " samples:\n*  ", e,
                  call. = FALSE
          )
        }
...
#'   row, with columns corresponding to variables)
...
test_multinormality <- function(values, test = "Mardia", threshold = 0.05, data, center, warnings = FALSE) {
  ...
  p.value <- withCallingHandlers(
    withRestarts({
      ...
      if (!exists("p.value") || is.na(p.value) || is.null(p.value)) {
        if (warnings) {
          warning("In test_multinormality() with test=\"", test, "\" and ", length(values),
                  " samples: p.value is NA\n",
                  call. = FALSE
          )
        }
...
#' @param data Matrix of numerical values containing the observations (one per
#'   row, with columns corresponding to variables)
#' @param test Statistical test used (user-defined function)
#' @param center Center of the observations from `data`
#' @param distances Unidimensional array of numerical values (distances of the
#'   observations to `center`)
#' @param warnings Logical value, to display the warnings and errors caught
#'
...
#' @seealso [filter_outliers()]
#'
test_custom <- function(data, test, center = NULL, distances = NULL, warnings = FALSE) {

  if (!is.function(test)) {
    stop("Parameter `test` should be a valid function")
  }
  ...
  p.value <- withCallingHandlers(
    withRestarts({
      ...
      p.value <- test(data,center,distances)


      # Check values are correct if it restarts from a warning
      if (!exists("p.value") || is.na(p.value) || is.null(p.value)) {
        if (warnings) {
          warning("In test_custom() with user-defined function and ", nrow(data),
                  " samples: p.value is NA\n",
                  call. = FALSE
          )
        }
    ...
    warning = function(w) {
      if (warnings) {
        warning("Warning caught in test_custom() with user-defined function and ",
                nrow(data), " samples:\n*  ", w,
                call. = FALSE
        )
      }
    ...
    error = function(e) {
      if (warnings) {
        if (warnings) {
          warning("Error caught in test_custom() with user-defined function and ",
                  nrow(data), " samples:\n*  ", e,
                  call. = FALSE
          )
        }
...
#'
#' @param x An object of class `BRIC.Filtering` (see [filter_outliers()])
#' @param maxDisplayed Number of elements to display in the output (default:
#'   200). Set to NULL (or 0) to show all values.
#' @param ... Other arguments passed to or from other methods
#'
#' @seealso [filter_outliers()], [plot.BRIC.Filtering()], [clustBRIC()]
#'
#' @export
#'
print.BRIC.Filtering <- function(x, maxDisplayed = 200, ...) {
  if (class(x) != "BRIC.Filtering") {
    stop("the object provided is not of class \"BRIC.Filtering\"")
  }
  ...
  cat(sprintf(
    ...
    ifelse(is.function(x$call$test),"User-defined",x$call$test), ifelse(is.function(x$call$test),"",x$call$testType),
    x$call$threshold, ifelse(x$call$distType == "Euclidean", "Euclidean", paste(x$call$distType, "-based Robust", sep = ""))
  ))
  cat(sprintf(
  ...
  cat("\n\n", ifelse(is.function(x$call$test),"User-defined",toupper(x$call$test)), " Test p.values:\n", sep = "")
...
#'
#
#' @seealso [filter_outliers()], [print.BRIC.Filtering()], [clustBRIC()],
#'   [median_rec()], [median_mv()], [depth_values()]
#'
#' @examples
#'
...
#' @export
#'
plot.BRIC.Filtering <- function(x, contents = c("p.values", "scatterplot", "dist", "hist"), showCenter = TRUE,
  ...
  while (length(contents) >= 1) {
    if (any(unlist(lapply(c("scater", "scatter", "plot"), grepl, contents[1])))) {
      ...
      title <- ifelse("main" %in% names(otherArgs), list(otherArgs$main),
                      paste(ifelse(is.function(x$call$test),"User-defined",x$call$test), "Test"))
      subtitle <- ifelse("sub" %in% names(otherArgs), list(otherArgs$sub),
                         ifelse(is.function(x$call$test),
                                paste0("P.Values for User-defined Test (p < ", x$call$threshold, ")"),
