...
#' packages \href{https://CRAN.R-project.org/package=depth}{`depth`} and
#' \href{https://CRAN.R-project.org/package=ddalpha}{`ddalpha`}) or convex body
#' minimizers (package \href{https://CRAN.R-project.org/package=MASS}{`MASS`}).
#'
...
#'
#' @return The function returns an S3 object of type `BRIC.MedianRec`,
#'   containing the following values: \item{`median`}{Coordinate of the
#'   recursive median} \item{`max`}{Coordinate of the sample with the highest
#'   depth (or the center of the first iteration in the case of convex body
#'   minimizers)} \item{`iterations`}{List containing the indices from the
#'   samples of `data` selected at each iteration}
#'
...
#'
#' @seealso [plot.BRIC.MedianRec()], [print.BRIC.MedianRec()], [median_mv()],
#'   [depth_values()], [clustBRIC()]
#'
#' @examples
#'
#' # Illustrative data
...
#'
#' @param x An object of class `BRIC.MedianRec` (see [median_rec()])
#' @param ... Other arguments passed to or from other methods
#'
#' @seealso [median_rec()], [plot.BRIC.MedianRec()]
#'
#' @export
#'
print.BRIC.MedianRec <- function(x, ...) {
  if (class(x) != "BRIC.MedianRec") {
...
#'
#' @seealso [median_rec()], [median_mv()], [clustBRIC()]
#'
#' @export
#'
plot.BRIC.MedianRec <- function(x, nbIterations = 5, showMedian = FALSE, showMax = FALSE, ...) {
  if (!is.logical(showMedian) || !is.logical(showMax)) {
    stop("showMedian and showMax must be logical value")
  }
  if (class(x) != "BRIC.MedianRec") {
    stop("the object provided is not of class \"BRIC.MedianRec\"")
  }
